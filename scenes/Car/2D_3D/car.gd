...
########## 2D , 3D
var dimensiones  #### here 2 2D , 3 3D
#prticules
#var particule_scene = preload("res://Prticule/Particles.tscn")
# controls
...
func _physics_process(delta):
	###### camera_2D, position.
	var pos = translation
	get_parent().get_node("2D").translation = Vector3(pos.x,pos.y,-35)
	if dimensiones == 2:
		get_parent().get_node("Back_gound_2D").translation = Vector3(pos.x,pos.y,5)
...
func _process(delta):
	...
	if total>0.0:
		...
		debugger.get_node("weight_dist").text = "weight distribution: "+str(distrr)+str("r /")+str(distrf)+str("f")

	
	debugger.get_node("rpm").text = "rpm: "+str(rpm)
	debugger.get_node("gear").text = "gear: "+str(gear)
	debugger.get_node("speed").text = "kmh: " +str(int(linear_velocity.length()*1.10130592))
	debugger.get_node("gforce").text = "gforce: " +str(float(gforce.length()))
	debugger.get_node("dlock").text = "diff_locked: " +str(float(locked))
	debugger.get_node("psi").text = "turbo psi: " +str(float(turbopsi))

	debugger.get_parent().get_node("sw").rect_rotation = 380.0*steer
	

func camera_dimensiones(numero):#### 2 , 3
	if numero ==2:
		dimensiones = 2
		get_parent().get_node("2D").current = true
		get_parent().get_node("cam_chase/orbit/Camera").current = false
	if numero ==3:
		dimensiones = 3
		get_parent().get_node("2D").current = false
		get_parent().get_node("cam_chase/orbit/Camera").current = true

func _on_Area_body_entered(body):
	camera_dimensiones(3)
	get_parent().get_node("3D/muro_Inicio/Animation_1").play("Muro")
	get_parent().get_node("Mapa_2D_3D").visible = false
	pass # Replace with function body.


func _on_Area_2_body_entered(body):
	get_parent().get_node("2D").projection = Camera.PROJECTION_PERSPECTIVE
	get_parent().get_node("Back_gound_2D/AnimationPlayer").play("desvanecer")
