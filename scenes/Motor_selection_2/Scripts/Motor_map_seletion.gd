...
func _process(delta):
	...
	if Dominio>= time:
		...
		var y_2 = (192.9175)/(1+0.0183*exp(0.0237*time))
		...
		var y_5 = (200)/(1+0.0321*exp(0.0117*time))  ### true
...
func _evaluacion_punto(dominio):
	if dominio<= 600 and dominio> -600 : 
		...
		var y_2 = (192.9175)/(1+0.0183*exp(0.0237*dominio))  
		...
		var y_5 = (200)/(1+0.0321*exp(0.0117*dominio))  ### true
		$Motor/Motor_container/Motor_graficos5._punto_position(dominio,y_5)
		
	pass

func _ready():
	$Plano.p_llegada_x = 4000
	$Plano.p_llegada_y = 5 
	$Plano.p_Inicio_y = 5
	$Plano.p_Inicio_x = 0
...
func _on_cortina_animation_finished(anim_name):
	...
	if Next_Rally_1 == true:
		Next_Rally_1 = false
		get_tree().change_scene("res://scenes/Motor_selection_3/Rally_1/Rally_1.tscn")
	pass # Replace with function body.
