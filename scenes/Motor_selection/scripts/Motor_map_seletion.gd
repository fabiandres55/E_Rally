...
func _evaluacion_punto(dominio):
	if dominio<= 600 and dominio> -600 : 
		var y = (192.9175)/(1+0.0183*exp(0.0237*dominio)) ### true
		$Motor/Motor_container/Motor_graficos._punto_position(dominio,y)
		var y_2 =(87.1783)/(1+0.0006*exp(0.0126*dominio))
		$Motor/Motor_container/Motor_graficos2._punto_position(dominio,y_2)
		var y_3 =(50.00000000000025)/(1+0.000000000000006*exp(0.061*dominio))
...
func _ready():
	$Plano.p_llegada_x = 3000
	$Plano.p_llegada_y =  529
	$Plano.p_Inicio_x = 0
	$Plano.p_Inicio_y = 0
	#####################################
	...
	#
	$Motor/Motor_container/Motor_graficos4.find_node("Bola").modulate = Color(1, 0, 0)
	$Motor/Motor_container/Motor_graficos4.find_node("trayecto").modulate = Color(1, 0, 0)
...
func _on_cortina_animation_finished(anim_name):
	...
	if Next_Rally_1 == true:
		Next_Rally_1 = false
		get_tree().change_scene("res://scenes/Motor_selection/Rally_1/Rally_1.tscn")
	pass # Replace with function body.
...
func _selec_motor(num):
	...
	if num == 0:
		...
		Motor_Type = [3.250, 1.894, 1.259, 0.937, 0.9]  # 3.250, 1.894, 1.259, 0.937, 0.9
	elif num == 1:
		$Motor/Motor_container/Motor_graficos2/Button_2.pressed = true
		Motor_Type = [2.0,2.0,2.0,2.0, 2.0]
	elif num == 2: 
