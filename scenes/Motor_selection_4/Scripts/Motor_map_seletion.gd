...
func _evaluacion_punto(dominio):
	if dominio<= 600 and dominio> -600 : 
		...
		var y_2 = (150)/(1+0.0183*exp(0.0237*dominio))
		$Motor/Motor_container/Motor_graficos2._punto_position(dominio,y_2)
		
		var y_3 = (200)/(1+0.0006*exp(0.0126*dominio))### true
...
func _ready():
	$Plano.p_llegada_x =  60
	$Plano.p_llegada_y = 30
	$Plano.p_Inicio_y = 20
	$Plano.p_Inicio_x = 15
	...
	#
	$Motor/Motor_container/Motor_graficos4.find_node("Bola").modulate = Color(0.827451, 1, 0)
	$Motor/Motor_container/Motor_graficos4.find_node("trayecto").modulate = Color(0.827451, 1, 0)
...
func _on_cortina_animation_finished(anim_name):
	...
	if Next_Rally_1 == true:
		Next_Rally_1 = false
		get_tree().change_scene("res://scenes/Motor_selection_4/Rally_1/Rally_1.tscn")
	pass # Replace with function body.
