...
#		var dist_cache = dist*(stress/5.0 +1.0)
		var dist_cache = dist
		
#		dist_cache /= 2.0
		var tol = 50.0*car.ClutchGrip
		var tol2 = 50.0*car.ClutchGrip*car.BiteMultiply
...
func _physics_process(_delta):
	...
	velocity = $velocity.global_transform.basis.orthonormalized().xform_inv($velocity.linear_velocity)
	velocity2 = $velocity2.global_transform.basis.orthonormalized().xform_inv($velocity2.linear_velocity)
	...
	var tyre_maxgrip = TyreSettings["GripInfluence"]
	...
	if braked>1.0:
		braked = 1.0
	var bp = (B_Torque*braked)/w_weight
	if bp>0.0:
	...
	if is_colliding():
		if "drag" in get_collider():
			drag = get_collider().get("drag")*CompoundSettings["GroundDragAffection"]
		...
		if "ground_builduprate" in get_collider():
			ground_builduprate = get_collider().get("ground_builduprate")
		...
		if ground_bump_up:
			ground_bump -= rand_range(ground_bump_frequency/ground_bump_frequency_random,ground_bump_frequency*ground_bump_frequency_random)*velocity.length()
			if ground_bump<0.0:
				ground_bump = 0.0
		...
		distx -= atan2(abs(wv),1.0)*(angle*10.0)
		...
		if grip>0:
			...
			if sqrt(pow(abs(disty),2.0) + pow(abs(distx),2.0))/(tyre_stiffness/3.0)>(car.ABS[0]/grip)*(ground_friction*ground_friction) and car.ABS[3] and velocity.length()>car.ABS[2] and ContactABS:
...
#			skvol *= skvol
	...
	var g = ($geometry.translation.y+(abs(cast_to.y) -A_Geometry1))/(abs(translation.x)+A_Geometry3 +1.0)
