...
func _process(_delta):
	...
	if not current_trail == null:
		if inserting:
			...
			if len(verts)>0:
				verts[len(verts)-1][0] = (ppos.origin + ppos.basis.orthonormalized().xform(Vector3(wid,0,0)))
				verts[len(verts)-1][1] = (ppos.origin - ppos.basis.orthonormalized().xform(Vector3(wid,0,0)))
				verts[len(verts)-1][2] = ppos.origin
