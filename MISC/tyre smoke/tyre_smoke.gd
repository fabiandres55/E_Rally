...
func _physics_process(delta):
	...
	$revolvel.translation.x = float(wheel_self.TyreSettings["Width (mm)"]) *0.0030592
	$revolver.translation.x = -float(wheel_self.TyreSettings["Width (mm)"]) *0.0030592
	
	$static.global_rotation = velo1.global_rotation
	var velo1_v = velo1.global_transform.basis.orthonormalized().xform_inv(velo1.linear_velocity)
	...
	if wheel_self.is_colliding():
		if dirt_type:
			if wheel_self.ground_dirt:
				if velo1.linear_velocity.length()>20.0:
					$static/lvl1.emitting = true
					if abs(wheel_self.wv*wheel_self.w_size)>velo1.linear_velocity.length()+10.0:
